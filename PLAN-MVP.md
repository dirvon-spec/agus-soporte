...
### 2.15 CATEGORÍAS FUERA DEL BALANCE — gate del dueño 4-sep-2026 (hallazgo de auditoría + decisiones del dueño)

Una auditoría posterior al cierre de §2.14 detectó que cualquier categoría con clientes sumaba de lleno a los agregados del negocio sin distinción. El dueño aprobó poder dejar categorías puntuales (cuentas propias, pruebas, casos especiales) FUERA de esos agregados sin sacarlas de la vista de trabajo diaria de Clientes.

**Esquema (schema_version 4 → 5):** `categorias` gana `modo_resumen TEXT NOT NULL DEFAULT 'NORMAL' CHECK (modo_resumen IN ('NORMAL','NO_SUMA','OCULTA'))`. Migración v4→v5 (`MIGRACION_V4_A_V5`): un único `ALTER TABLE ADD COLUMN` con CHECK de una sola columna (permitido por SQLite, mismo caso que v1→v2) — array plano de SQL sin backfill en JS. Aplicada en `initDb()`/`migrarEsquemaSiHaceFalta()` y en `importarRespaldo()` (encadenada después de v1→v2/v2→v3/v3→v4 para archivos más viejos). Todas las categorías existentes quedan en `NORMAL` por el DEFAULT, sin tocar datos.

1. **Global (decisión 1):** la omisión (NO_SUMA u OCULTA) saca a la categoría de TODO agregado del negocio — Balance general (Clientes y Global), las tarjetas Abonos/Cargos del mes y los totales del calendario diario de Global. La exclusión se hace en `obtenerCalendarioGlobalMovimientos` con un `LEFT JOIN` a `categorias` (`cat.id IS NULL OR cat.modo_resumen = 'NORMAL'`), así que el calendario y las tarjetas del mes de Global quedan consistentes entre sí por construcción (se derivan de las mismas filas ya filtradas). `carteraPendienteCentavos` sigue viniendo de `resumenMensual()` SIN filtrar por categoría (reporte completo intacto, no se toca ningún consumidor existente) y hoy no se muestra en ninguna UI.
2. **"Sin categoría" (decisión 2):** SIEMPRE cuenta, como NORMAL fijo. No es una fila de `categorias` (es el balde especial `SIN_CATEGORIA` de `listarClientesAgrupados`), así que no tiene un `modo_resumen` que se le pueda cambiar — no se puede omitir.
3. **OCULTA en Clientes (decisión 3):** sus grupos van a una sección colapsable "📦 Ocultas del resumen (n)" al pie de la lista de Clientes, colapsada por defecto — el cliente sigue ahí, accesible para cobrarle con normalidad. NO_SUMA en cambio queda visible en su posición normal de la lista, con un badge "no suma" junto al nombre de la categoría (título del grupo y fila Σ).
4. **Transparencia (decisión 4):** cuando hay al menos un grupo fuera del balance, aparece una línea bajo el Balance general — en Clientes y en Global — con el formato "Balance real: $X · N fuera del balance" (`calcularBalanceReal`/`contarGruposFueraDeBalance` en `js/ui/componentes.js`, calculados sobre TODOS los grupos sin excluir nada).

**Invariante que no se rompe:** los totales POR GRUPO (fila Σ, tanto en la UI como en `grupo.totales` de `listarClientesAgrupados`) siguen siendo REALES para las tres — NORMAL, NO_SUMA y OCULTA. Lo único que cambia es la participación en los AGREGADOS; `grupoParticipaEnBalance(grupo)` (`componentes.js`) es la única función que decide esa participación, y trata la ausencia del campo (contrato anterior a §2.15) como NORMAL — nunca esconde datos por un dato ausente.

**Capa de datos:** `crearCategoria`/`actualizarCategoria` aceptan `modo_resumen` (`MODOS_RESUMEN = ['NORMAL','NO_SUMA','OCULTA']`, default NORMAL, `VALIDATION_ERROR` si no es uno de esos tres). `listarClientesAgrupados` propaga `categoria_modo_resumen` en cada grupo y, en modo-día, excluye a las categorías fuera del balance de `resumenDia.cobradoCentavos`/`abonaron`/`dijeronNo`/`sinVisitar` — el cliente igual conserva su `estado_dia` real en su propia fila (el semáforo no miente, solo se excluye de las tarjetas agregadas).

Verificación mínima: crear una categoría con cada modo_resumen y ver el badge/sección correctos en Clientes; un cliente NO_SUMA con abono hoy no mueve ninguna de las 3 tarjetas de Clientes pero sí su propia fila y la Σ de su grupo; una categoría OCULTA desaparece de Global (calendario y tarjetas del mes) pero su cliente sigue cobrable desde "Ocultas del resumen"; la línea "Balance real: $X · N fuera del balance" aparece solo cuando corresponde y desaparece con 0 categorías fuera; migración v4→v5 sobre una base real preserva todas las categorías existentes en NORMAL; import de un respaldo v1..v4 sigue funcionando (encadena hasta v5); suite completa verde.
